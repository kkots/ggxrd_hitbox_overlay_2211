...
void Graphics::dumpTextures(IDirect3DDevice9* device) {
	...
	getModuleBounds("GuiltyGearXrd.exe:all", &start, &end, &wholeModuleBegin, nameBuffer, sectionBuffer);
	...
	TextureArrayDesc textureArrayDescs[] {
		{ 0x114, L"StaticTextures" },
		{ 0x118, L"LoadedStaticTexture" },
		{ 0x11c, L"External0" },
		{ 0x120, L"External1" },
		{ 0x124, L"External2" },
		{ 0x128, L"External3" }
	};
	for (int textureArrayIteration = 0; textureArrayIteration < (int)_countof(textureArrayDescs); ++textureArrayIteration) {
		REDTexture2DArray* textureArray = *(REDTexture2DArray**)(gamePtr + textureArrayDescs[textureArrayIteration].offset);
		if (textureArray == nullptr) return;
		for (int i = 0; i < textureArray->ArrayNum; ++i) {
			TextureArrayItem* elem = textureArray->Data + i;
			std::wstring name = TEXTUREDUMP_PATH;
			name += textureArrayDescs[textureArrayIteration].name;
			name += L"\\";
			...
			if (GetFileAttributesW(name.c_str()) != INVALID_FILE_ATTRIBUTES) continue;
